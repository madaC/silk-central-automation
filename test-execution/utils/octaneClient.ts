...
const getTestContainerAppModule = async (
    applicationModuleIds: string[]
): Promise<OctaneApplicationModule> => {
    const query = Query.field('id')
        .inComparison(applicationModuleIds)
        .and(Query.field('source_type_udf').equal('test container'));
    const octaneResponse = await octane
        .get(Octane.entityTypes.applicationModules)
        .fields('id', 'name', 'source_type_udf', 'sc_source_control_udf')
        .query(query.build())
        .execute();
    return <OctaneApplicationModule>octaneResponse.data[0];
};
